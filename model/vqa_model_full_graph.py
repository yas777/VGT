...
class POSEmbeddings(nn.Module):
    ...
    def forward(self, embeddings, cid):
        ...
        embeddings = self.merge_pos(torch.cat(
            [embeddings, position_embeddings], dim=-1))  # (bs, max_seq_length, dim)
...
class VGT(nn.Module):
    def __init__(
        ...
        self.cm_satt_pool = copy.deepcopy(self.satt_pool)

        self.bert_pool = nn.Sequential(nn.Linear(
            d_model, d_model//2), nn.Tanh(), nn.Linear(d_model//2, 1), nn.Softmax(dim=-2))

        self.gnn = Graph(dim_in=d_model, dim_hidden=d_model//2,
                         dim_out=d_model, num_layers=4, dropout=dropout)
        self.gnn1 = Graph(dim_in=d_model, dim_hidden=d_model//2,
                          dim_out=d_model, num_layers=4, dropout=dropout)
        ...
        self.ftrans = Transformer(self.config)
        ...
        self.shared_projection = nn.Linear(d_model, 1)
        self.bilstm_pool_regions = BiLSTM(d_model, 100, 2, d_model)
        self.bilstm_pool_frames = BiLSTM(d_model, 100, 2, d_model)
        self.bilstm_pool_clips = BiLSTM(d_model, 100, 2, d_model)
    ...
    def get_vqa_embedding_rfcpos(self, video, language, language_lens):
        ...
        xlen = numc*numr*numf
        # print("X before CM attention: ", X.shape, xlen)
        # X = X.view(bsize, xlen, -1)
        X = X.reshape(bsize, xlen, -1)
        # print("X after View Change: ", X.shape)
        X = self.cm_interaction(X, xlen, language, language_lens,
                                self.bidirec_att_regions, self.bilstm_pool_regions, ans_n)
        # print("X after CM attention: ",X.shape)

        X = X.view(bsize, numc, numr, numf, -1)
        ### End of node attention ###
        short_mask = get_mask(torch.tensor(
            [numf]*bsize*numc*numr, dtype=torch.long), numf).cuda()
        ...
        graph_mask = get_mask(torch.tensor(
            [numf]*bsize*numc, dtype=torch.long), numf).cuda()
        # The pretrained model on webvid use N=10 region proposals, to adapt to NExT-QA
        # which has different number of regions, please change the layer_name to a new one, e.g., graph_trans_n
        ...
        #Full size graph
        X1 = X_o.view(bsize, numc*numr*numf, -1)
        short_mask = get_mask(torch.tensor(
            [numf*numc*numr]*bsize, dtype=torch.long), numf*numc*numr).cuda()
        X1 = self.ftrans(X1, short_mask)[0]
        A = self.gnn1.build_graph(X1)  # (bs, numc*numf*numr, numc*numf*numr)
        A = F.softmax(A, dim=-1)
        X_o = self.gnn1(X1, A)

        # satt = self.satt_pool(X_o)
        # X_o = torch.sum(X_o*satt, dim=-2)
        X_o = self.bilstm_pool_regions(X_o)
        ...
        video = self.cm_interaction(
            video, xlen, language, language_lens, self.bidirec_frames, self.bilstm_pool_frames, ans_n)
        ...
        video = self.cm_interaction(
            video, numc, language, language_lens, self.bidirec_att, self.bilstm_pool_clips, ans_n)
        ##### cross-model attention#############
        return video

    def cm_interaction(self, X, xlen, language, language_lens, cm_attention, pooler, ans_n=5):
        bsize, nclips, d_model = X.shape[0], X.shape[1], X.shape[-1]
        X_len = torch.tensor([xlen] * bsize, dtype=torch.long).to(X.device)
        if ans_n == 1:
            Xatt, _ = self.bidirec_att(
                X, X_len, language, language_lens.view(-1))
        else:
        ...
        Xatt += X
        Xatt = Xatt.view(bsize*ans_n, xlen, -1)
        # sattn_weights = self.cm_satt_pool(Xatt)
        # Y = torch.sum(Xatt*sattn_weights,dim=-2).squeeze() # clip embedding (bsize, ans, d_model)
        Y = pooler(Xatt).view(bsize, ans_n, -1)
        Y = self.shared_projection(Y).squeeze()  # (bsize, ans)
        Y = F.gumbel_softmax(Y, dim=-1, hard=True)
        Y = Y.unsqueeze(dim=2).expand(bsize, ans_n, nclips).unsqueeze(
            dim=3).expand(bsize, ans_n, nclips, d_model)
        Xatt = Xatt.view(bsize,ans_n,xlen,-1)
        X = torch.sum((Xatt*Y), dim=1)
    ...
    def forward(
        ...
        if mode == "vqa":
            ...
            if self.Q != 0:
                ...
                video_proj = self.get_vqa_embedding_rfcpos(
                    video, question_w, seq_len)
            ...
            if text.shape[1] < max_seq_len:
                text = torch.cat(
                    [
                        text,
                        torch.zeros(
                            text.shape[0], max_seq_len -
                            text.shape[1], text.shape[2]
                        ).cuda(),
                    ],
                    1,
                )
            ...
            mlm_loss = self.mlm_loss_fct(
                prediction_logits.view(-1,
                                       prediction_logits.size(-1)), labels.view(-1)
            )
            return mlm_loss
